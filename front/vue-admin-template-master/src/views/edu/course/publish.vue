...
export default {
  ...
  methods: {
    ...
    //根据课程id查询课程及其他信息
    getCouseInfoById(){
      course.getCouseInfoById(this.courseId)
        .then(response => {
           this.publishCourse =  response.data.data
        })
    },
    goPublishCourse(){
      course.goPublishCourse(this.courseId)
        .then(response => {
              //提示
              this.$message({
                  type: 'success',
